...
function exit_mess()
	...
	file_log:write(string.format("%s Заявки на покупку:\n", os.date()))
	for ind, val in ipairs(order_requests_buy) do
		PrintDbgStr(string.format("vrfma: Индекс: %i Значение: %s", ind, tostring(val)))
		file_log:write(string.format("	Индекс: %i Значение: %s\n", ind, tostring(val)))
	end	
	for key, val in pairs(order_numbers_buy) do
		PrintDbgStr(string.format("vrfma: Ключ: %s Значение: %s", tostring(key), tostring(val)))
