...
function OnInit()	-- событие - инициализация QUIK
	...
	PrintDbgStr("vrfma_test: Событие - инициализация QUIK")
...
function test_boop_par(bool_par)	-- событие - остановка скрипта
	if bool_par then
		PrintDbgStr("vrfma_test: bool_par = true")
	else
		PrintDbgStr("vrfma_test: bool_par = false")
	end
	PrintDbgStr(string.format("vrfma_test: bool_par: %s", tostring(bool_par)))
end
...
function exit_mess()
	...
	PrintDbgStr("vrfma_test завершён")
