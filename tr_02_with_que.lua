...
function exit_mess()
	...
	PrintDbgStr("tr_02: Заявки: ")
	file_log:write(string.format("%s Заявки:\n", os.date()))
	for ind, val in ipairs(order_requests) do
		PrintDbgStr(string.format("tr_02: Индекс: %i Значение: %s", ind, tostring(val)))
		file_log:write(string.format("	Индекс: %i Значение: %s\n", ind, tostring(val)))
	end	
	for key, val in pairs(order_numbers) do
		PrintDbgStr(string.format("tr_02: Ключ: %s Значение: %s", tostring(key), tostring(val)))
		file_log:write(string.format("	Ключ: %s Значение: %s\n", tostring(key), tostring(val)))
	end
	PrintDbgStr("tr_02: tr_02 завершён")
...
function OnTrade(trade)	-- событие - QUIK получил сделку
	for _, ord in pairs(order_numbers) do
		if ord == trade.order_num then
			PrintDbgStr(string.format("tr_02: Событие - QUIK получил сделку order_num - %s", trade.order_num))
